...
const SOCIAL = [
  ...
  {
    href: "https://open.spotify.com/show/048IHm26dtJt5QwI3vOxcd?si=qfatvqRUTYSbKNrClaBhQA",
    icon: Mic2,
    label: "Podcast",
  },
];
    ...
    <footer className="bg-navy-dark border-t border-white/10">
      <div className="bg-gradient-to-r from-rust via-amber-site to-gold py-5 px-6 text-center">
        <p className="font-accent text-navy text-sm tracking-widest uppercase">
          Daily Prayer Watch · 12:00 AM – 2:00 AM CST · Join on Telegram, Mixlr
          & YouTube
        </p>
      </div>
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-3 gap-12">
        <div>
          <div className="font-accent text-gold text-xs tracking-widest2 uppercase mb-1">
            Esther Adetoun
          </div>
          <div className="font-display text-white text-2xl font-light mb-4">
            Adeyeye
          </div>
          <div className="w-10 h-px bg-gold mb-4" />
          <p className="text-white/50 text-sm leading-relaxed font-body">
            Heralding a Revival Movement through Prayer, the Word & the Power of
            the Holy Spirit.
          </p>
          ...
          <ul className="space-y-3">
            {LINKS.slice(0, 5).map(({ to, label }) => (
              <li key={to}>
                <Link
                  to={to}
                  className="text-white/50 text-sm hover:text-gold transition-colors font-body"
                >
                  {label}
                </Link>
              </li>
      ...
      <div className="border-t border-white/10 py-6 px-6 flex flex-col sm:flex-row items-center justify-between gap-3">
        <p className="text-white/30 text-xs font-body">
          © {new Date().getFullYear()} Esther Adetoun Adeyeye. All rights
          reserved.
        </p>
